...
with open(sys.argv[1],'r') as outputFile:
    data = outputFile.readlines()

for line in data:
    line = line.strip('\n').split(' ')
        #print(line)
    if (line[0] == 'HAM') and ("spam" in line[1]):
        fn['spam'] += 1
    elif (line[0] == 'SPAM') and ("ham" in line[1]):
        fp['spam'] += 1
    elif (line[0] == 'SPAM') and ("spam" in line[1]):
        tp['spam'] = tp['spam'] + 1


    if (line[0] == 'SPAM') and ("ham" in line[1]):
        fn['ham'] += 1
    elif (line[0] == 'HAM') and ("spam" in line[1]):
        fp['ham'] += 1
    elif (line[0] == 'HAM') and ("ham" in line[1]):
        tp['ham'] += 1
